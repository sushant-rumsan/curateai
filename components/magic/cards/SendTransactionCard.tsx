...
const SendTransaction = () => {
  ...
  const sendTransaction = useCallback(async () => {
    ...
    if (isEip1559Supported()) {
      ...
      txnParams.maxFeePerGas = BigInt(feeData.maxFeePerGas as number);
      txnParams.maxPriorityFeePerGas = BigInt(feeData.maxPriorityFeePerGas as number);
    } else {
      txnParams.gasPrice = await web3.eth.getGasPrice() as unknown as string;
    }
