...
import { useReadCuratePostsPosts, useReadCurateTokenBalanceOf, useWriteCuratePostsVote } from "@/hooks/wagmi/contracts"
import { CONTRACT } from "@/constants/contract"
import { useState } from "react"
import { useAccount } from "wagmi"
import { Loader2, AlertCircle, Heart, MessageSquare, Share2, Bookmark, Coins } from "lucide-react"
          ...
          <header className="mb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">Posted on {new Date(data.date).toLocaleDateString()}</div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 rounded-lg p-6">
            ...
            <form onSubmit={handleScore} className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button type="submit" disabled={isScorePending} className="px-6">
                  {isScorePending ? (
                    <>
        ...
        <aside className="space-y-6">
          <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <div className="text-gray-500 text-sm">{postData ? `${postData[1].toString().slice(0, 8)}...${postData[1].toString().slice(-8)}` : "Anonymous"}</div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <h2 className="font-bold px-6 py-4 border-b border-gray-200">More from CurateAI</h2>
            <div className="divide-y divide-gray-200">
              {[1, 2, 3].map((i) => (
                <a key={i} href="#" className="block p-6 hover:bg-gray-50 transition-colors">
                  <h3 className="font-medium text-gray-900 hover:text-blue-600 mb-1">
                    Another interesting blockchain post
                  </h3>
                  <div className="text-sm text-gray-600">{new Date().toLocaleDateString()} • 5 min read</div>
                </a>
              ))}
            </div>
          </div>
